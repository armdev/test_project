...
@Component
public class PortfolioFileService extends DbInitBean implements Serializable {
    ...
    public PortfolioFileService() {
        log.info("Backend: STARTED PortfolioFileService");
    }
    ...
    public String addFile(FileEntity file) {
        ...
        try {
            gfsPhoto = new GridFS(initDatabase.getDatabase(), "portfoliofiles");
            ...
            Long fileId = (CommonUtils.longValue(getNextId(initDatabase.getDatabase(), "portGenSeq")));
    ...
    public GridFSDBFile getFile(String fileName) {
        try {
            gfsPhoto = new GridFS(getDatabase(), "portfoliofiles");
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void removeFileByFileName(String imageName) {
        ...
        initDatabase.getPortfolioFileCollection().remove(document);
    }
